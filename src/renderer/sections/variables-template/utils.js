...
export const removeItemById = (data, idToRemove) => {
  ...
  data.children = data.children
    ...
    .map((child) =>
      child.children ? removeItemById(child, idToRemove) : child,
    );

  return data;
};

export const findParentAndRemove = (
  data,
  idToRemove,
  removeAssignedVariable,
) => {
  ...
  for (let i = 0; i < data.children.length; i++) {
    ...
    if (child.id === idToDuplicate) {
      ...
      if (duplicatedChild.children) {
        duplicatedChild.children = duplicatedChild.children.map(
          (grandChild) => {
            const newGrandChild = {
              ...grandChild,
              config: {
...
export const updateItemConfigById = (
  data,
  idToUpdate,
  fieldKey,
  fieldValue,
  ...
  for (let i = 0; i < data.children.length; i++) {
    ...
    if (child.children) {
      const result = updateItemConfigById(
        child,
        idToUpdate,
        fieldKey,
        fieldValue,
