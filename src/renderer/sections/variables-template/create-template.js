...
import useUndoRedoVariableUi from '../../hooks/use-undo-redo-variable-ui';
  ...
  return (
    <Stack
      className={`item-flow-template ${
        selectingItem?.id === id ? 'isSelecting' : ''
      }`}
      sx={{
      ...
      >
        ...
        <Stack
          sx={{
            ...
            height:
              (level !== 0 &&
                showValueHasMinMax([100, 1000], config?.height)) ||
              'auto',
            minHeight: '100px',
                  ...
                  >
                    {ContentComponent && (
                      <ContentComponent
                        ...
                        onDuplicate={onDuplicate}
                        onDelete={onDelete}
                        isMissingVariable={componentValidate.includes(c.id)}
                      />
                    )}
                  </Stack>
                );
...
const CreateVariablesTemplate = React.memo(
  ({
    ...
    const [componentValidate, setComponentValidate] = useState([]);
    ...
    const updateGroupOrder = (id, group) => {
      ...
      if (_findParrent?.name === 'Group') {
        ...
        if (checkAnyGrids) {
          alert('The maximum number of grid is 1');
          return false;
        }
      }
    ...
    const onDelete = (id) => {
      ...
      setComponentValidate((prev) => prev.filter((i) => i !== id));
    ...
    const getTooltipTitle = useCallback(() => {
      ...
      if (wfInfo?.source_workflow) {
        title = 'Not available for source workflow';
      } else if (wfInfo?.is_encrypted) {
        title = 'Not available for encrypted workflow';
      }
      return title;
    ...
    useEffect(() => {
      if (designData?.children?.length > 0) {
        const getAssignedVariable = (data) => {
          if (data.type === 'GROUP') {
            data.children.forEach((item) => {
              if (item.type === 'ITEM') {
                if (item.config?.variable?.id) {
                  setAssignedVariable((prev) => [
                    ...prev,
                    item.config.variable.id,
                  ]);
                }
              } else {
    ...
    }, [ruleset, tabId, variableTemplateMode]);

    useEffect(() => {
      const validateComponent = (data) => {
        setComponentValidate(data);
      };
      eventBus.on('validate-component', validateComponent);

      return () =>
        eventBus.removeListener('validate-component', validateComponent);
    }, []);
            ...
            {selectingItem && (
              ...
              >
                <Sidebar
                  ...
                  setComponentValidate={setComponentValidate}
                />
              </Stack>
            )}
          </Stack>
