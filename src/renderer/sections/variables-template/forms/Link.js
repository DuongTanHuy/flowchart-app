...
const Link = ({ template, updateItemByField, variableOptions }) => (
  <Stack>
    ...
    <CustomLabel nameLabel="">
      <FormControlLabel
        ...
        control={
          <Switch
            name="openNewTab"
            checked={template?.config?.openNewTab}
            onChange={(e) =>
              updateItemByField(template, 'openNewTab', e.target.checked)
            }
          />
        }
      />
    ...
    <CustomLabel nameLabel="Display text">
      <TextField
        ...
        onChange={(e) =>
          updateItemByField(template, 'displayText', e.target.value)
        }
        inputProps={{
          autoCorrect: 'off',
          spellCheck: 'false',
