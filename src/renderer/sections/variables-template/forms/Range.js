...
const Range = ({ template, updateItemByField, variableOptions }) => {
  ...
  useEffect(() => {
    if (
      Number(template?.config?.rangeMin) > Number(template?.config?.rangeMax)
    ) {
      setValidate(true);
    } else {
      setValidate(false);
    ...
    if (
      Number(template?.config?.defaultMin) >
      Number(template?.config?.defaultMax)
    ) {
    ...
    <Stack>
      ...
      <CustomLabel nameLabel="Variables">
        <Autocomplete
          ...
          options={
            variableOptions.filter((item) => item?.type === 'range') || []
          }
      ...
      <Stack spacing={1}>
        <FormControl>
          <FormLabel id="input-accept-type-label">Range Type</FormLabel>
          <RadioGroup
            row
            aria-labelledby="input-accept-type-group"
            value={template?.config?.type}
            name="type"
            onChange={(e) => {
              updateItemByField(template, 'type', e.target.value);
            }}
            sx={{
              display: 'grid',
            ...
            <CustomLabel nameLabel="Label">
              <Stack direction="row" spacing={2}>
                <TextField
                  ...
                  label="Min"
                  placeholder="Enter value"
                  value={template?.config?.minLabel}
                  onChange={(e) => {
                    updateItemByField(template, 'minLabel', e.target.value);
                  }}
                  inputProps={{
                    autoCorrect: 'off',
                    spellCheck: 'false',
                ...
                <TextField
                  ...
                  label="Max"
                  placeholder="Enter value"
                  value={template?.config?.maxLabel}
                  onChange={(e) => {
                    updateItemByField(template, 'maxLabel', e.target.value);
                  }}
                  inputProps={{
                    autoCorrect: 'off',
                    spellCheck: 'false',
            ...
            <FormControlLabel
              ...
              control={
                <Switch
                  ...
                  onChange={(e) =>
                    updateItemByField(template, 'marks', e.target.checked)
                  }
                />
              }
            />
            ...
            <FormControlLabel
              ...
              control={
                <Switch
                  ...
                  onChange={(e) =>
                    updateItemByField(template, 'tooltip', e.target.checked)
                  }
                />
              }
            />
      ...
      {template?.config?.type === 'slider' && (
        <CustomLabel nameLabel="Step">
          <TextField
            ...
            onChange={(e) =>
              updateItemByField(template, 'step', e.target.value)
            }
            inputProps={{
              autoCorrect: 'off',
              spellCheck: 'false',
