...
const Sidebar = ({
  ...
  const updateItemByField = (item, fieldName, content) => {
    if (fieldName === 'variable') {
      ...
      if (content) {
        if (item?.config?.variable?.id)
          newAssignedVariable = removeNumberOnce(
            newAssignedVariable,
            item?.config?.variable?.id,
          );

        setAssignedVariable([...newAssignedVariable, content?.id]);
        setComponentValidate((prev) => prev.filter((i) => i !== item.id));
      } else {
        newAssignedVariable = removeNumberOnce(
          newAssignedVariable,
          item?.config?.variable?.id,
    ...
    const afterUpdated = updateItemConfigById(
      ruleset,
      item.id,
      fieldName,
      content,
...
Sidebar.propTypes = {
  ...
  setComponentValidate: PropTypes.func,
};
