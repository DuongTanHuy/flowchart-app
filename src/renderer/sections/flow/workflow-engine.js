...
class WorkflowEngine {
  ...
  async init() {
    try {
      ...
      if (this.nodeStart) {
        const targetBlockId = this.checkReverse(
          this.nodeStart,
          nodes,
          edges,
          true,
      ...
      const triggerBlock = nodes.find(
        (node) => node.id === (this.triggerBlockId ?? 'start_node'),
      );
      ...
      this.connectionsMap = edges.reduce(
        (acc, { source, sourceHandle, target, targetHandle }) => {
  ...
  checkReverse(nodeId, listNodes, listEdges, skip = false) {
    ...
    if (
      listEdges.find(
        (edge) => edge.target === nodeId && edge.sourceHandle === 'fallnext',
      )?.id
    ) {
      return '';
    }
