...
import { cleanedJSCode, replaceObjectValuesWithKeys } from '../../utils/rpa';
import {
  extractBracedValues,
  generateCondition,
  generateLogicScript,
...
class WorkflowWorker {
  ...
  isValidEndLoop(path, endLoopId) {
    return path.some((nodeId) =>
      this.engine.workflow.drawflow.nodes.find(
        (node) =>
          node.id === nodeId &&
          ['lap_phan_tu', 'lap_du_lieu'].includes(node?.data?.alias),
      ),
    );
  }
  ...
  traverseAndCheck(nodeId, path = []) {
    ...
    for (const childId of children) {
      const childNode = this.engine.workflow.drawflow.nodes.find(
    ...
    switch (block?.dataFields?.method) {
      case 'find':
        this.engine.setScript(
          ...
          }", [ITEM => ${this.applySyntax(
            generateCondition(block?.dataFields?.conditions),
          )}]);`,
        );
        break;
      case 'filter':
  ...
  formatFieldsAndApplySyntax(array) {
    return array.reduce((obj, item) => {
      if (item.key)
        obj[this.applySyntax(item.key)] = this.applySyntax(item.value);
      return obj;
    }, {});
  }
              ...
              if (
                ...
                if (content) {
                  ...
                  const currentLoop = this.engine.workflow.drawflow.nodes.find(
                    (item) =>
                      item.type === 'loop' &&
                      item.dataFields.loop_id === loopId,
                  );
  ...
  updateBlockDataFields(block, dataFields) {
    switch (block?.data?.alias) {
      case 'http_request':
        dataFields = {
          ...
          ...(dataFields.content_type === 'text'
            ? {
                data:
                  typeof dataFields.data === 'string' ? dataFields.data : '',
              }
            : {
                data:
      ...
      case 'code': {
        ...
        const parts = dataFields.js_code
          .split(regex)
          .filter((part) => part && part.trim());
        const result = parts.map((part) => {
      ...
      case 'element_exists':
        dataFields = {
          ...dataFields,
          element_selector: dataFields.element_selector?.replace(
            /^(['"`])(.*?)(\1)$/,
            '$2',
          ),
        };
      ...
      case 'chuyen_frame':
        dataFields = {
          ...dataFields,
          selector_val: dataFields.selector_val?.replace(
            /^(['"`])(.*?)(\1)$/,
            '$2',
          ),
        };
      ...
      case 'facebook_api':
        dataFields = {
          ...dataFields,
          category_ids: dataFields.category_ids?.map(
  ...
  executeNextBlocks(connections, prevBlockData) {
    ...
    const successHandle = connections.find(
      ...
      case 'diem_cuoi_vong_lap':
        if (!this.listErrorNode.includes(block.id)) {
          ...
          if (cond) {
            ...
            //     this.engine.workflow?.variables,
            //     block.id,
            //   ),
            // );
            // this.engine.setScript(
      ...
      case 'dieu_kien': {
        ...
        const paths = connections?.nextBlockId?.filter(
          (connection) =>
            ...
            connection?.sourceHandle !== 'fallnext',
        );
      ...
      default:
        if (block?.id !== 'start_node') {
          if (
            ...
            let xxx = generateLogicScript(
              block.data?.script_template,
              dataFields,
            );
