...
export default function UploadFileForm({ formData, IdNode }) {
  ...
  const getVariable = (name, item) => {
    ...
    if (name === 'path') {
      ...
      if (
        ...
        finalValue = `${formData?.dataFields?.path.slice(
          0,
          selectionStart,
        )}\${${item.key}}${formData?.dataFields?.path.slice(selectionEnd)}`;
      } else {
  ...
  const selectGoogleCreden = async (event) => {
    if (isElectron()) {
      ...
      const path = (isElectron() ? file?.path || '' : file?.name || '').replace(
        /\\/g,
        '/',
      );
      eventBus.emit('updateNode', {
    ...
    <Stack>
      ...
      <Stack spacing={2} mt={2}>
        ...
        <FormControlLabel
          ...
          control={
            <Checkbox
              checked={formData?.dataFields?.is_upload_folder ?? false}
            />
          }
          onChange={handleChangeNumberSecond}
        ...
        <FormControlLabel
          ...
          control={
            <Checkbox
              checked={formData?.dataFields?.is_upload_by_dialog ?? false}
            />
          }
          onChange={handleChangeNumberSecond}
