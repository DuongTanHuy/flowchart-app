...
import { RESOURCE_OPTIONS } from '../../../utils/constance';
...
export default function UpdateResourceForm({ formData, IdNode }) {
  ...
  useEffect(() => {
    if (formData?.dataFields?.data?.length !== 0) {
      setOptions(
        OPTIONS.map((item) => {
          const _find = formData?.dataFields?.data?.findIndex(
            (i) => i.key === item.value,
          );
          return {
            ...item,
    ...
    <Stack>
      ...
      <Stack spacing={2} mt={2}>
        ...
        <Stack sx={styleOptions} spacing={1} mt={1}>
          ...
          >
            ...
            <Button
              ...
              disabled={
                options?.length === 0 ||
                formData?.dataFields?.data?.length >= 12
              }
            >
              Thêm
            </Button>
          ...
          <Stack
            spacing={1.5}
            pt={formData?.dataFields?.data?.length > 0 ? 2 : 0}
          >
              ...
              >
                ...
                {item.key === 'status' && (
                  <Typography
                    ...
                    pl={1}
                  >
                    Chỉ chấp nhận các trạng thái sau: live, die, checkpoint,
                    not_logged_in, wrong_password
                  </Typography>
                )}
              </Stack>
            ))}
        ...
        <FormControlLabel
          ...
          control={
            <Checkbox
              checked={formData?.dataFields?.is_create_when_not_exists ?? false}
            />
          }
          onChange={handleChangeNumberSecond}
