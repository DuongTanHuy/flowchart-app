  ...
  const getVariable = (name, item) => {
    ...
    if (name === 'selector') {
      ...
      if (
        ...
        finalValue = `${formData?.dataFields?.selector.slice(
          0,
          selectionStart,
        )}\${${item.key}}${formData?.dataFields?.selector.slice(selectionEnd)}`;
      } else {
      ...
      <Stack spacing={3} mt={2}>
        <Stack spacing={1}>
          ...
          <Typography
            ...
            fontSize={14}
          >
            * Nếu không chọn &quot;Loại bộ chọn&quot;, ứng dụng sẽ nhập văn bản
            vào vị trí con trỏ hiện tại trên trình duyệt.
          </Typography>
        </Stack>
