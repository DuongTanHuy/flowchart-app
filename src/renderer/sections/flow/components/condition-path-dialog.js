    ...
    switch (value) {
      ...
      case 'element_attribute_value':
        return `element#attribute${
          item.leftValue ? `(${item.leftValue})` : ''
        } ${item.operator?.title || 'Trống'} ${item.rightValue || 'Trống'}`;
    ...
    if (conditions.length > 0) {
      ...
      conditions.forEach((condition, ind) => {
        if (
          ...
          condition.conditions?.[0]?.leftValue
        ) {
          let sentence = '';
          condition.conditions.forEach((item, index) => {
            if (index === 0) {
              ...
              sentence += ` [VÀ] ${handleGetPathTitle(
                item.leftType?.value,
                item,
              )}`;
            }
