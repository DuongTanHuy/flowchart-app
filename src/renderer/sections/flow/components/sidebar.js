    ...
    const getScriptOptions = async () => {
      ...
      for (let i = 0; i < listFlowOption.length; i++) {
        if (listFlowOption[i].parent === null) {
          ...
          const filterChild = listFlowOption.filter(
            (op) => op.parent === item.id,
          );
    ...
    for (let i = 0; i < listSuggestion.length; i++) {
      ...
      if (item?.options) {
        item.options = item?.options.map((op) => ({
          ...op,
          display:
            !['notification', 'export_data', 'element_exists'].includes(
              op.alias,
            ) &&
  ...
  useEffect(() => {
    ...
    return () => {
      eventBus.removeListener(
        'responsiveSibarFlowchart',
        handleResponsiveSibarFlowchart,
      );
    };
  }, []);
      ...
      >
        <Stack sx={{ height: '100%' }}>
          <GeneralFormDialog
            ...
            nodes={
              flowAutomation?.typeForm?.alias === 'nhan_phim' ? nodes : null
            }
          />
                      ...
                      (trig) =>
                        trig.display && (
                          <Stack
                            ...
                            className={
                              responsiveSidebar !== null &&
                              responsiveSidebar[0] < 233
                            ...
                            onDragStart={(event) =>
                              onDragStart(event, {
                                ...
                                isHighlighted: false,
                                type:
                                  (['lap_du_lieu', 'lap_phan_tu'].includes(
                                    trig.alias,
                                  ) &&
                                    'loop') ||
                                  (trig.alias === 'lap_co_dieu_kien' &&
                                    'condition_loop') ||
                                  (trig.alias === 'cookies' && 'cookies') ||
                                  (trig.alias === 'element_exists' &&
                                    'element_exists') ||
                                  // (trig.alias === 'lap_co_dieu_kien' && 'while_loop') ||
                                  (trig.alias === 'diem_cuoi_vong_lap' &&
                                    'end_loop') ||
                                  (trig.alias === 'dieu_kien' &&
                                    'conditions') ||
                                  'custom',
                              })
                            }
                            ...
                            >
                              ...
                              <Tooltip title="Ghim">
                                <Iconify
                                  ...
                                  onClick={() => {
                                    const index =
                                      listSuggestion[0].options.findIndex(
                                        (option) => option.id === trig.id,
                                      );
                                    if (index > -1) {
                                      listSuggestion[0].options.splice(
                                        index,
                                        1,
                                      );
                                    } else {
...
const areEqual = (prevProps, nextProps) => {
  ...
  return prevProps.nodes.every((prevNode, index) => {
    ...
    return (
      JSON.stringify(prevNode.dataFields) ===
      JSON.stringify(nextNode.dataFields)
    );
  });
};
