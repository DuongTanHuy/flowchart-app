...
export default function ScrollMouseForm({ formData, IdNode }) {
  ...
  const handleInputChange = (event) => {
    eventBus.emit('updateNode', {
      data: {
        speed: event.target.value === '' ? 0 : Number(event.target.value),
      },
      idNode: IdNode,
    });
  };
  ...
  const getVariable = (name, item) => {
    ...
    if (name === 'element_selector') {
      ...
      if (
        ...
        finalValue = `${formData?.dataFields?.element_selector.slice(
          0,
          selectionStart,
        )}\${${item.key}}${formData?.dataFields?.element_selector.slice(
          selectionEnd,
        )}`;
      } else {
