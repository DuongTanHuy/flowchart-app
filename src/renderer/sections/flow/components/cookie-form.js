...
const OPTIONS = [
  ...
  { label: 'Xóa', value: 'clear' },
];

// ----------------------------------------------------------------------

export default function CookieForm({ formData, IdNode }) {
    ...
    <Stack>
      ...
      <Stack spacing={3} mt={2}>
        <TextField
          ...
          name="action_type"
          label="Hành động"
          value={formData?.dataFields?.action_type || 'import'}
        ...
        {formData?.dataFields?.action_type === 'import' && (
          <TextField
            fullWidth
            onChange={handleChangeNumberSecond}
            value={formData?.dataFields?.cookie_str || ''}
            name="cookie_str"
            label="Chuỗi cookie"
            InputProps={{
              endAdornment: (
        ...
        {formData?.dataFields?.action_type === 'export' && (
          <Autocomplete
            name="variable_name"
            disablePortal
            id="combo-box-demo"
