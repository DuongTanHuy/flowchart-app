...
export default function ReadFileForm({ formData, IdNode }) {
  ...
  const getVariable = (name, item) => {
    ...
    if (
      ...
      finalValue = `${formData?.dataFields?.file_path.slice(
        0,
        selectionStart,
      )}\${${item.key}}${formData?.dataFields?.file_path.slice(selectionEnd)}`;
    } else {
        ...
        {formData?.dataFields?.mode === 'one_line' && (
          <>
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={formData?.dataFields?.is_delete_after_read || false}
                />
              }
              onChange={handleChangeNumberSecond}
