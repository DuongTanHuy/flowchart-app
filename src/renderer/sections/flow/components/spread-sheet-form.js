...
import { convertCSVToJson } from '../../../utils/format-file';
...
export default function SpreadSheetForm({ formData, IdNode }) {
  ...
  const selectFileCSV = (event) => {
    ...
    eventBus.emit('updateNode', {
      data: {
        file_path: (isElectron() ? file?.path || '' : file?.name || '').replace(
          /\\/g,
          '/',
        ),
      },
  ...
  useEffect(() => {
    if (currentFile) {
      handleCSVInputChange(
        currentFile,
        dataFields?.is_choose_first_rows_as_key,
      );
    }
  ...
  const selectGoogleCreden = async (event) => {
    if (isElectron()) {
      ...
      const credentialJson = await window.ipcRenderer.invoke(
        'read-file-json',
        file?.path,
      );
      eventBus.emit('updateNode', {
  ...
  const handleReviewSheetData = async () => {
    ...
    if (dataFields?.spreadsheet_type === 'excel') {
      ...
      if (isElectron()) {
        ...
        const path_credentials_json = getParameterValue(
          dataFields.credentials_json,
          globalData,
        );
          ...
          <Stack spacing={3}>
            ...
            {['read', 'clear'].includes(dataFields?.action_type) && (
              <TextField
                ...
                name="range"
                label={`Range ${
                  dataFields?.action_type === 'clear' ? '' : '(Tùy chọn)'
                }`}
                onChange={handleChangeNumberSecond}
            ...
            {['write', 'append'].includes(dataFields?.action_type) && (
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField label="Biến dữ liệu" {...params} />
                )}
            ...
            {['read', 'write', 'append'].includes(dataFields?.action_type) && (
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    checked={
                      formData?.dataFields?.is_choose_first_rows_as_key ?? true
                    }
                  />
                }
                onChange={handleChangeNumberSecond}
            ...
            {dataFields?.action_type === 'update' && (
              ...
              >
                ...
                <Stack
                  spacing={1}
                  pt={dataFields?.data_update?.length > 0 ? 2 : 0}
                >
          ...
          <Stack spacing={3}>
            <Stack>
              ...
              >
                ...
                <Typography
                  ...
                  onClick={() =>
                    referLink(
                      'https://docs.mktlogin.com/guide/setup-google-sheet.html',
                    )
                  }
                >
                  click vào đây
            ...
            <Stack>
              <Typography
                ...
                fontSize={14}
              >
                Hãy copy email dưới và share bảng tính của bạn cho email đã
                copy.{' '}
                <Typography
                  component="span"
                  style={{
                  ...
                  onClick={() =>
                    referLink(
                      'https://docs.mktlogin.com/guide/how-to-share-sheetspread.html',
                    )
                  }
                >
                  Xem hướng dẫn
            ...
            {['update', 'delete_row', 'find_row'].includes(
              dataFields?.action_type,
            ) && (
              <Stack spacing={3} direction="row">
                <TextField
                  fullWidth
