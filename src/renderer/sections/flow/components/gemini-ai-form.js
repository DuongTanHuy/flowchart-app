...
export default function GeminiAiForm({ formData, IdNode }) {
  ...
  const getVariable = (name, item) => {
    ...
    if (
      ...
      finalValue = `${formData?.dataFields?.image_attachment.slice(
        0,
        selectionStart,
      )}\${${item.key}}${formData?.dataFields?.image_attachment.slice(
        selectionEnd,
      )}`;
    } else {
  ...
  const selectGoogleCreden = async (event) => {
    if (isElectron()) {
      ...
      const image_attachment = (
        isElectron() ? file?.path || '' : file?.name || ''
      ).replace(/\\/g, '/');
      eventBus.emit('updateNode', {
      ...
      <Stack spacing={3} mt={2}>
        ...
        {formData?.dataFields?.model === 'custom' && (
          <TextField
            ...
            InputLabelProps={{
              shrink: !!data.model_custom || focusField.model_custom,
            }}
            onFocus={() =>
              setFocusField((prev) => ({ ...prev, model_custom: true }))
            }
            onBlur={() =>
              setFocusField((prev) => ({ ...prev, model_custom: false }))
            }
