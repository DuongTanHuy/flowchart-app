...
import CustomMenuContext from '../custom-popover/custom-menu-context';
import { LoadingScreen } from '../loading-screen';

import useUndoRedoFlow from '../../hooks/use-undo-redo-flow';
...
import CreateVariablesTemplate from '../../sections/variables-template/create-template';
import {
  NodeFlowchartValidation,
  generateFields,
  getConnectedNodes,
  sortEdgesForLayoutAttempt,
} from '../../utils/handle-bar-support';
...
import AlertDialogAdvanced from '../ask-before-leave-advanced';
import SettingAutomation from '../automation-setting';
import '../../sections/flow/style.css';
import customNode from '../../sections/flow/components/custom-node';
import customStartNode from '../../sections/flow/components/custom-start-node';
import customCookieNode from '../../sections/flow/components/custom-cookie-node';
import customConditionLoopNode from '../../sections/flow/components/custom-condition-loop-node';
import customLoopNodeV2 from '../../sections/flow/components/custom-loop-node-v2';
import customEndLoop from '../../sections/flow/components/custom-end-loop';
import customConditionNode from '../../sections/flow/components/custom-condition-node';
import customEdge from '../../sections/flow/components/custom-edge';
import customElementExistNode from '../../sections/flow/components/custom-element-exist-node';
...
import ButtonBar from '../../sections/flow/button-bar';
import Iconify from '../iconify';
import FlowButton from '../../sections/flow/flow-button';
import FlowChartView from '../../sections/flow/flow-view';
import CustomConnectionLine from '../../sections/flow/components/connectionLine';
import WorkflowRunningConfig from '../../sections/flow/workflow-running-config';
import Sidebar from '../../sections/flow/components/sidebar';
...
export default function FlowView({ tabData, tabId }) {
  ...
  const [, setNodeSelected] = useState('');

  const [, setTriggerId] = useState(null);
  ...
  const [designData, setDesignData] = useState(
    tabData?.designData ?? initialGroup,
  );
  ...
  const [sidebarAble, setSidebarAble] = useState(
    defaultGroupInVisible?.flow
      ? [0, 'auto']
      : sidebarFlowchart?.flowchart ?? [300, 'auto'],
  );
  ...
  const handlePanelClick = useCallback(() => {
    ...
    const highlightNode = reactFlowInstance
      ?.getNodes()
      .find((item) => item.data?.isHighlighted);
    if (highlightNode?.id) {
      setNodeSelected('');
  ...
  const handlePasteNode = useCallback(
    () => {
      ...
      if (Array.isArray(copiedNode)) {
        ...
        const selectedGroupNodes = copiedNode.filter(
        ...
        const newGroupNodes = selectedGroupNodes.map((groupNode) => {
          if (groupNode?.parentId) {
            ...
            const parentNode = listNodes.find(
              (item) => item.id === groupNode.parentId,
            );
            if (parentNode) {
        ...
        copiedNode
          .filter(
            (node) =>
              node.parentId && copiedNode.find((n) => n.id === node.parentId),
          )
          .forEach((childNode) => {
            ...
            const originalParentNode = nodes.find(
            ...
            if (newParentId && originalParentNode && newParentNode) {
              const newChildNode = createPastedNode(
                childNode,
                basePosition,
                newParentId,
                {
        ...
        const childNodes = reactFlowInstance
          ?.getNodes()
          .filter(
            (node) => node.parentId === copiedNode.id && node.type !== 'group',
          );
        ...
        const childEdges = reactFlowInstance
          ...
          .filter(
            (edge) =>
              groupedNodeIds.includes(edge.source) &&
              groupedNodeIds.includes(edge.target),
          );
    ...
    (type = 'simple') => {
      ...
      if (type === 'simple') {
        ...
        if (targetNode) {
          if (targetNode?.id.split('_')[0] === 'group') {
            const childNodes = reactFlowInstance
              ...
              .filter(
                (node) =>
                  node.parentId === targetNode.id && node.type !== 'group',
              );
        ...
        const selectedNodes = nodes.filter(
          (node) => node.selected && node.id !== 'start_node',
        );
        ...
        if (selectedNodes.length > 0) {
          ...
          selectedNodes
            .filter(
              (node) =>
                node.parentId &&
                selectedNodes.find((n) => n.id === node.parentId),
            )
            .forEach((childNode) => {
              const originalParentId = childNode.parentId;
              const newParentId = duplicatedGroupsMap[originalParentId];
  ...
  const alignNodes = (direction) => {
    const { nodes: layoutedNodes, edges: layoutedEdges } =
      applyDagreLayoutOnSelectedNodes(nodes, edges, direction);
  ...
  const deleteNode = useCallback(
    (type = 'simple') => {
      const deleteNodesAndEdges = (nodeIdsToDelete) => {
        setNodes((listNode) => {
          const newNodes = listNode.filter(
            (node) => !nodeIdsToDelete.has(String(node.id)),
          );
          setHistory({
            nodes: newNodes,
            edges,
        ...
        setEdges((listEdge) =>
          listEdge.filter(
            (edge) =>
              !nodeIdsToDelete.has(String(edge.source)) &&
              !nodeIdsToDelete.has(String(edge.target)),
          ),
        );
      };
  ...
  const groupNode = useCallback(() => {
    ...
    const hasNonGroupedNode = selectedNodes.some(
      (node) => !node.parentId && node.type !== 'group',
    );
    const hasMoreThanOneGroup =
      selectedNodes.filter((node) => node.type === 'group').length > 1;
    ...
    const updatedNodes = currentNodes.map((node) => {
      if (
        selectedNodes.some(
          (selected) => selected.id === node.id && !node.parentId,
        )
      ) {
        return {
          ...node,
    ...
    () => [
      {
        label: 'Sao chép',
        icon: 'tabler:copy',
        action: () => handleCopyNode('multiple'),
      },
      {
        label: 'Nhân bản',
        icon: 'ion:duplicate-outline',
  ...
  const onToggleFullScreen = useCallback(() => {
    if (!document.fullscreenElement) {
      ...
      settings.onUpdate('isFullScreen', true);
  ...
  const traverseAndCheckReverse = useCallback(
    (nodeId) => {
      ...
      if (
        ['lap_co_dieu_kien', 'lap_phan_tu', 'lap_du_lieu'].includes(
      ...
      const parents = reactFlowInstance
        ?.getEdges()
  ...
  const onConnect = useCallback(
    (paramsmeter) => {
      ...
      if (findPath(target, source)) {
        enqueueSnackbar('Reverse connection!', {
          variant: 'error',
          anchorOrigin: {
            vertical: 'top',
            horizontal: 'center',
      ...
      if (
        ...
        if (!isValidLoop) {
          enqueueSnackbar('Invalid loop connection!', {
            variant: 'error',
            anchorOrigin: {
              vertical: 'top',
              horizontal: 'center',
      ...
      setEdges((eds) => {
        ...
        if (paramsmeter.source === 'start_node' && isStartNodeConnected) {
          enqueueSnackbar(
            'The start node has been connected to another action!',
            {
              variant: 'error',
              anchorOrigin: {
                vertical: 'top',
        ...
        const newList = addEdge(
          {
            ...
            style: {
              strokeWidth: 1.3,
              stroke:
                (paramsmeter.sourceHandle === 'error' &&
                  theme.palette.error.main) ||
                (['condition-else', 'outer'].includes(
                  paramsmeter.sourceHandle,
                ) &&
            ...
            markerEnd: {
              ...
              color:
                (paramsmeter.sourceHandle === 'error' &&
                  theme.palette.error.main) ||
                (paramsmeter.sourceHandle === 'condition-else' &&
                  theme.palette.warning.main) ||
                theme.palette.success.main,
            },
            // type: 'smoothstep',
  ...
  const onDrop = useCallback(
    (event) => {
      try {
        ...
        if (
          typeof transferredData?.type === 'undefined' ||
          !transferredData?.type
        ) {
          return;
        }
  ...
  const handleHighlightNode = useCallback(
    (nodeId) => {
      ...
      const currentHighlightedNode = nodes_list.find(
        (item) => item.data?.isHighlighted,
      );
  ...
  const validateFlowChart = useCallback(
    (listNode, listEdge, action = 'save') => {
      ...
      if (connectedNode.length === 0 || connectedNode[0] === undefined) {
        enqueueSnackbar('Please create at least one action for your process!', {
          variant: 'error',
          anchorOrigin: {
            vertical: 'top',
            horizontal: 'center',
      ...
      if (errorStatus || connectedNode.length === 0) {
        enqueueSnackbar(
          action === 'save'
            ? 'Please enter complete data for your process before saving!'
            : 'Please enter complete data for your process before running!',
          {
            variant: 'error',
            anchorOrigin: {
              vertical: 'top',
  ...
  const saveFlowChart = useCallback(
    async (ruleset) => {
      ...
      if (design_data?.children?.length > 0) {
        handleValidateComponent(design_data, errors);
      }

      if (errors.length > 0) {
        setVariableTemplateMode('designer');
        eventBus.emit('validate-component', errors);
        return false;
      }
  ...
  const applyDagreLayoutOnSelectedNodes = useCallback(
    (listNodes, listEdges, direction = 'LR') => {
      ...
      sortEdges.forEach((edge) => {
        const sourceGroupId = listNodes.find(
          (n) => n.id === edge.source,
        )?.parentId;
        const targetGroupId = listNodes.find(
          (n) => n.id === edge.target,
        )?.parentId;
      ...
      const firstLayoutNode = dagreGraph.node(
        firstGroup.parentId || firstGroup.id,
      );
      ...
      const xOffset =
        firstGroup.position.x - (firstLayoutNode.x - firstNodeWidth / 2);
      const yOffset =
        firstGroup.position.y - (firstLayoutNode.y - firstNodeHeight / 2);
          ...
          if (parentNode) {
            updatedPositions[currentNode.parentId] = {
              x:
                ...
                nodeDragStartPosition.current.x,
              y:
                parentNode.position.y +
                currentNode.position.y -
                nodeDragStartPosition.current.y,
            };
          }
        } else {
      ...
      if (node.parentId && previewNodePosition[node.parentId]) {
        reactFlowInstance?.setNodes((nds) => {
          const newList = nds.map((n) =>
            n.id === node.parentId
              ? { ...n, position: previewNodePosition[node.parentId] }
              : n,
          );
          setHistory({
            nodes: newList,
          ...
          <ReactFlowProvider>
            ...
            >
              {JSON.stringify(sidebarAble) === JSON.stringify([0, 'auto']) && (
                <Stack
                  sx={{ position: 'absolute', top: 60, left: 0, zIndex: 10 }}
                >
