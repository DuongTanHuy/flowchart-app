...
export const getOperator = (operator) => {
  switch (operator) {
    ...
    case 'contains_case_insensitive':
      return (substring) =>
        `?.toLowerCase().includes(\`${substring}\`?.toLowerCase())`;
    case 'not_contains':
      return (substring) => `?.indexOf(\`${substring}\`) === -1`;
    case 'not_contains_case_insensitive':
      return (substring) =>
        `?.toLowerCase().indexOf(\`${substring}\`?.toLowerCase()) === -1`;
    case 'starts_with':
...
export const detectBrowser = (userAgent) => {
  ...
  let browserName = Object.keys(browsers).find((browser) =>
...
export const handleValidateComponent = (data, errors) => {
  if (data.type === 'GROUP') {
    data.children.forEach((item) => {
      if (item.type === 'ITEM') {
        if (
          !item.config?.variable?.id &&
          !['Divider', 'Alert', 'Inline'].includes(item.name)
        ) {
          errors.push(item.id);
        }
      } else {
        handleValidateComponent(item, errors);
      }
    });
    ...
    if (
      !data.config?.variable?.id &&
      !['Divider', 'Alert', 'Inline'].includes(data.name)
    ) {
      errors.push(data.id);
    }
  }
