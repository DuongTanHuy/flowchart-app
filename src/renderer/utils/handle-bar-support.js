...
Handlebars.registerHelper('json', (context) => {
  const needsBracketNotation = Object.keys(context).some(
    (key) => /^\$\{[^}]+\}$/.test(key) || key.includes('${'),
  );

  const jsonString = JSON.stringify(context);

  const escapedQuotesReplaced = jsonString.replace(/"(\\".*?\\")"/g, '`$1`');

  if (needsBracketNotation) {
    // trường hợp này cần khi dùng trong node gg sheet update data, dùng biểu thức loop trong key update
    return escapedQuotesReplaced.replace(/"(.*?)":"(.*?)"/g, '[`$1`]:`$2`');
  }
...
export const generateCondition = (conditions) => {
  const condition = conditions.map((item) =>
    generateAndCondition(item?.conditions),
  );
  return condition.join(' || ');
};
...
export const getConnectedNodes = (startNodeId, listNodes, listEdges) => {
  ...
  return Array.from(visited).map((id) =>
    listNodes.find((node) => node.id === id),
  );
};
